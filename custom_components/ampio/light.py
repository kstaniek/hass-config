...
class AmpioLight(AmpioEntity, light.LightEntity):
    ...
    def __init__(self, config):
        ...
        # if self._config.get(CONF_WHITE_VALUE_COMMAND_TOPIC) is not None:
        #     self._attr_supported_color_modes = {ColorMode.BRIGHTNESS, ColorMode.RGBW}
    ...
    async def subscribe_topics(self):
        ...
        def brightness_received(msg):
            """Handler new MQTT message."""
            payload = msg.payload
            self._attr_brightness = int(payload)
            self._state = self._attr_brightness > 0
            self.async_write_ha_state()
        ...
        def rgb_received(msg):
            ...
            if any(rgbw):
                ...
                self._state = False
            #     self._attr_hs_color = color_util.color_RGB_to_hs(*rgb[:3])
            #     percent_bright = float(color_util.color_RGB_to_hsv(*rgb[:3])[2]) / 100.0
            #     self._attr_brightness = percent_bright * 255
            #     self._state = True
            # else:
            #     self._state = False
            # self._attr_rgbw_color = *rgb[:3], self._attr_white
        ...
        #     self._attr_white = float(msg.payload)
        #     if self._attr_white > 0:
        #         self._state = True
        #     self._attr_rgbw_color = *self._attr_rgbw_color[0:3], self._attr_white
        #     self.async_write_ha_state()
    ...
    async def async_turn_on(self, **kwargs):
        ...
        #     # )
        #     rgb_color_str = ",".join(map(str, self._attr_rgb_color))
        #     async_publish(
        ...
        if (ATTR_RGBW_COLOR in kwargs
            ...
            rgb_color_str = ",".join(map(str, self._attr_rgbw_color))
            async_publish(
                self.hass, self._config[CONF_RGB_COMMAND_TOPIC], rgb_color_str, 0, False
            )
            return
        ...
        if self._config.get(CONF_COMMAND_TOPIC) is not None:
            async_publish(
                self.hass, self._config[CONF_COMMAND_TOPIC], 255, 0, False
            )
        elif self._config.get(CONF_RGB_COMMAND_TOPIC) is not None:
            async_publish(
                self.hass, self._config[CONF_RGB_COMMAND_TOPIC], "255,255,255,0", 0, False
            )
